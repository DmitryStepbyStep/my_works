...
{   
    ...
    public class UnitTest1
    {
...

        {
            ...
            string elementCategoryId = driver.FindElement(By.CssSelector("#CategoryId > option:nth-child(4)")).GetAttribute("text");
            string elementSupplierId = driver.FindElement(By.CssSelector("#SupplierId > option:nth-child(2)")).GetAttribute("text");
            ...
            //проверяем значения
            Assert.AreEqual(valueProductName, "Charlotte");
            Assert.AreEqual(elementCategoryId, "Confections");
            Assert.AreEqual(elementSupplierId, "Exotic Liquids");
            Assert.AreEqual(valueUnitPrice, "15,0000");
            Assert.AreEqual(valueQuantityPerUnit, "1kg");
            Assert.AreEqual(valueUnitsInStock, "163");
        ...
        [TestMethod]
        public void Test3ProductDelete_app()
        {
            IWebDriver driver = new ChromeDriver();
            driver.Url = "http://localhost:5000";
            LoginPage Login = new LoginPage(driver);
